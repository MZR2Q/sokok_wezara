...
(function () {
  ...
  const ROLES = [
    { role: "hq",       icon: "🏛️", title: "المركز الرئيسي (الوزارة)", desc: "متابعة جميع الفروع وإسناد الصكوك." },
    { role: "branch",   icon: "🏢", title: "مدير فرع",                 desc: "استقبال الصكوك وتوزيعها على الموظفين." },
    { role: "employee", icon: "👤", title: "موظف توثيق",               desc: "تنفيذ مسار توثيق الصكوك المسندة إليه." },
  ];
  ...
  ROLES.forEach((r) => {
    const card = el("div", { class: "role-card", "data-role": r.role, onclick: () => selectRole(r.role) }, [
      el("div", { class: "r-ic", text: r.icon }),
      el("div", {}, [ el("b", { text: r.title }), el("p", { text: r.desc }) ]),
      el("div", { class: "r-check", text: "✓" }),
    ]);
